...
export class HealthService {
   ...
   */
  async checkStellarHealth(horizonUrl: string, rpcUrl: string): Promise<HealthCheckResponse> {
    ...
    if (health.services.horizon.status === "DOWN" || health.services.sorobanRpc.status === "DOWN") {
  ...
  private async measurePing(fn: () => Promise<any>): Promise<{ latency: number; data: any }> {
  ...
  private formatStatus(result: PromiseSettledResult<{ latency: number; data: any }>): ServiceStatus {
    if (result.status === "fulfilled") {
      return {
        status: "UP",
        latencyMs: result.value.latency,
        ledgerVersion: result.value.data.sequence || result.value.data.records?.[0]?.sequence
      };
    }
    return {
      status: "DOWN",
